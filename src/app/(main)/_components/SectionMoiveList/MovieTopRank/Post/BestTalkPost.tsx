...
interface ReviewType {
  movieId?: number;
  reivewData: MovieReviewDTO;
}

export default function BestTalkPost({ reivewData, movieId }: ReviewType) {
  return (
          ...
          <>
            <div className="flex-2 flex flex-row items-center justify-center gap-1 Desktop:flex-col">
              <Image
                ...
                src={`data:image/jpeg;base64,${reivewData.profile}
            `}
                alt="영화 포스터"
              />
            ...
            <div className="flex items-center justify-center gap-1 Text-s-Medium">
              ...
              {reivewData.likeCount && <span>{reivewData.likeCount}</span>}
            </div>
          </>
        ) : (
          <>
